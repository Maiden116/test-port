...
describe('Stack operation test', function() {
    ...
    it('Pop, peek and swap action buttons clicks also should show an alert message “operation not allowed” if the stack board is empty.', async function() {
        peek.click();
        let alertText = await driver.switchTo().alert().getText();
        await driver.switchTo().alert().dismiss();
        expect(alertText).to.equal('Operation not allowed!')
        ...
        let alertText1 = await driver.switchTo().alert().getText();
        await driver.switchTo().alert().dismiss();
        expect(alertText1).to.equal('Operation not allowed!')

        swap.click();
        let alertText2 = await driver.switchTo().alert().getText();
        await driver.switchTo().alert().dismiss();
        expect(alertText2).to.equal('Operation not allowed!')
    });

    it('Clicking on the swap can swap two topmost elements of the stack ', async function() {
        input.sendKeys("item1");
        push.click();
        input.sendKeys("item2");
        push.click();
        ...
        let item5Text = await item5.getAttribute('innerText');
        expect(item5Text).to.equal('item4');
        driver.takeScreenshot().then(
            function(image, err) {
                require('fs').writeFile('swap-view.png', image, 'base64', function(err) {});
            }
        );
    });

    it('Clicking on pop will remove the top item from the stack.', async function() {
        input.sendKeys("item1");
        push.click();
        input.sendKeys("item2");
        push.click();
